...
class VWGCN(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k, embed_dim, dropout, add_apt):
        ...
        if self.add_apt:
            self.weights_pool = nn.Parameter(torch.FloatTensor(embed_dim, cheb_k, dim_in, dim_out)) #1 NAPL
        ...
        # mask = torch.eye(36)
        # self.mask = torch.logical_not(mask).type(torch.float32)
    ...
    def forward(self, x, node_embeddings, lap):
        ...
        # support_set = [torch.zeros(node_num, node_num).to(x.device), lap] # adj_mx -> lap
        # support_set = [lap]
        # default cheb_k = 3
        ...
        if self.add_apt: #torch.mm 两个二维的张量相乘(N,D)x(D,N)
            ...
            # self.mask = self.mask.to(supports.device)
        ...
        # GA-GCGRU，expand weights
        if self.add_apt:
            weights = torch.cat((weights,torch.zeros(weights.shape[0], 1, weights.shape[2], weights.shape[3]).to(weights.device)), 1) #NAPL去掉 cheb +1，保持矩陣正常計算
...
class AGCRNCell(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, dropout, add_apt):
        ...
        self.forget_input_output_gate = VWGCN(dim_in+self.hidden_dim, 3*dim_out, cheb_k, embed_dim, dropout, add_apt)
        self.graph_update = VWGCN(dim_in+self.hidden_dim, dim_out, cheb_k, embed_dim, dropout, add_apt)

    def forward(self, x, state, cell, node_embeddings, graph_lap=None):
        """
        modified GRU
        ...
        forget_input_output = torch.sigmoid(self.forget_input_output_gate(input_and_state, node_embeddings, graph_lap))  # B, num_nodes, 2 * hidden_dim
        _forget, _input, _output = torch.chunk(forget_input_output, chunks=3, dim=-1)  # B, num_nodes, hidden_dim

        candidate = torch.cat((x, state), dim=-1)  # B, num_nodes, input_dim + hidden_dim
        ct_ = torch.tanh(self.graph_update(candidate, node_embeddings, graph_lap))  # B, num_nodes, hidden_dim
        ct = _forget * cell + _input * ct_
        ht = _output * torch.tanh(ct)
        return ct, ht
...
class AVWDCRNN(nn.Module):
    ...
    def forward(self, x, init_state, init_cell, node_embeddings, graph_lap=None):
        """
        Multi LSTM
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):
                ...
                cell, state = self.dcrnn_cells[i](current_inputs[:, t, :, :], state, cell, node_embeddings, graph_lap)
    ...
    def init_cell(self, batch_size):
        init_cell = []
        for i in range(self.num_layers):
            init_cell.append(self.dcrnn_cells[i].init_cell_state(batch_size))
        return torch.stack(init_cell, dim=0)
...
class GCN_LSTM(nn.Module):
    def __init__(self, config, adj_mx=None):
        ...
        self.adj_mx = torch.tensor(calculate_normalized_laplacian(adj_mx)
    ...
    def forward(self, batch_x):  # (batch_size, input_window, num_nodes)
        inputs = batch_x.permute((0, 2, 1))    # (batch_size, num_nodes, input_window)
        inputs = torch.unsqueeze(inputs, dim=-1)  # (batch_size, num_nodes, input_window, 1)
        ...
        init_cell = self.encoder.init_cell(inputs.shape[0])
        # graph_lap = torch.tensor(calculate_normalized_laplacian(graph).astype(np.float32).todense()).to(inputs.device)
        output, _ = self.encoder(inputs, init_state, init_cell, self.node_embeddings, self.adj_mx)  # B, T, N, hidden
        output = output[:, -1:, :, :]
        #  (B, 1, N, hidden_dim)
        if self.dropout_rate != 0:
    ...
    def get_NAPL_DAGG(self):
        return self.encoder.dcrnn_cells[0].graph_gate.weights_pool, \
               self.encoder.dcrnn_cells[0].graph_update.weights_pool,\
               self.encoder.dcrnn_cells[0].graph_gate.ada_DAGG, \
               self.encoder.dcrnn_cells[0].graph_update.ada_DAGG,\
               self.encoder.dcrnn_cells[1].graph_gate.weights_pool, \
               self.encoder.dcrnn_cells[1].graph_update.weights_pool,\
