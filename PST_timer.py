...
def train(config, dataset):
    ...
    model = get_model(config, graph_dict['graph_1']).to(config.device)
    ...
    for epoch in range(config.epoch):
        ...
        log.info(f"Epoch={epoch}, exp_id={config.exp_id}, seq_Loss: {pred_loss:.7f}")
...
def main():
    ...
    parser.add_argument("--model", type=str,
                        default='MTGNN')  # GCN_GRU, TCN_GCN, GCN_TCN, GCN_LSTM, MTGNN, AGCRN, TCN_Mixhop, Dilated_GCN, Parallel_MTGNN, FCSTGNN
    parser.add_argument("--batch_size", type=int, default=32)  # 16 32
    parser.add_argument("--epoch", type=int, default=100)
    parser.add_argument("--search_best_range", type=list, default=[69, 70])
    ...
    datasets = ["./config/config_test1.yaml", "./config/config_test2.yaml"]
    # models = ['GCN_GRU', 'GCN_LSTM', 'MTGNN', 'AGCRN']
    models = ['Parallel_MTGNN']

    for ds in datasets:
        for mn in models:
            config.conf = ds
            config = edict(yaml.load(open(config.conf), Loader=yaml.FullLoader))
            config.update(dict_args)
            config.var_len = config.data_diff + 1
            config['exp_id'] = str(exp_id)
            config.model = mn
            ...
            Sensor_test(config, dataset)
            pred_end_time = time.time()

            train_time = (train_end_time - train_start_time) / 60
            test_time = (pred_end_time - pred_start_time) / 60

            with open('./save_result/time_graph.txt', mode='a+') as f:
                f.write(f'{ds}--{mn}:train-{train_time:.6f}min,test-{test_time:.6f}min')
