...
def main(dict_para):
    ...
    parser.add_argument("--model", type=str,
                        default='Parallel_MTGNN')  # GCN_GRU, TCN_GCN, GCN_TCN, GCN_LSTM, MTGNN, AGCRN, TCN_Mixhop, Dilated_GCN, Parallel_MTGNN, FCSTGNN
...
if __name__ == '__main__':
    SA_para = 'graph_file'
    para = {'gcn_k': [1, 2, 3, 4, 5], 'data_diff': [1, 2, 3, 4, 5], 'dilation_0': [1, 2, 3, 4, 5],
            'graph_file': [4, 6, 8, 10, 12]}
    all_result = []
    for value in para[SA_para]:
        if SA_para == 'graph_file':
            value = [f"48_node_train_graph_{value}.xlsx"]
        result_str, result_str_1, result_list = main({SA_para: value})
        all_result.append(result_list)
        with open(f'./save_result/sensitivity_analysis/{SA_para}.txt', 'a+') as file:
            file.write(result_str + '\n' + result_str_1 + '\n')
    all_result = np.array(all_result)
    df_result = pd.DataFrame(all_result, columns=['RMSE', 'MAE', 'MAPE', 'SMAPE', 'RAE'])
    df_result.to_csv(f'./save_result/sensitivity_analysis/{SA_para}.csv')
