...
class TCN(nn.Module):
    ...
    def forward(self, inputs):
        """Inputs have to have dimension (bs, in_dim, Node, seq_len)"""
...
class TCN_GCN(nn.Module):
    def __init__(self, config, adj_mx=None):
        ...
        self.tc1 = TCN(input_channels=self.in_channel, kernel_size=self.kernel_size, tcn_layers=self.num_channels, d0=self.d0)
        self.gc1 = Cheb_conv(self.gcn_depth, in_channels=self.num_channels[-1], out_channels=self.num_channels[-1] // 2,
                             device=self.device)
        ...
        self.end = nn.Conv2d(self.num_channels[-1] // 2, self.num_channels[-1] // 4, kernel_size=(1, 1))
        self.end_1 = nn.Conv2d(self.num_channels[-1] // 4, 1, kernel_size=(1, 2))
    ...
    def forward(self, batch_x):  # (32, 10, 24)
        ...
        output = self.tc1(batch_x)  # (32, 32, 24, 10)
        output = self.gc1(output, self.predefined_A)  # (bs, 16, 36, 8)
