...
class Parallel_MTGNN_Pz(nn.Module):
    def __init__(self, config, adj_mx=None):
        ...
        if self.input_window > self.receptive_field:
            self.end_conv = nn.Conv2d(in_channels=self.residual_channels,
                                   ...
                                   kernel_size=(1, self.input_window - self.receptive_field + 1 - (self.dilation_0 -1) * 6), bias=True)
        else:
            self.end_conv = nn.Conv2d(in_channels=self.residual_channels,
        ...
        self.embed_node_1 = nn.Conv2d(in_channels=self.num_nodes, out_channels=self.out_nodes//2, kernel_size=(1, 1))
        self.embed_node_2 = nn.Conv2d(in_channels=self.out_nodes//2, out_channels=self.out_nodes, kernel_size=(1, 1))
        self.relu = nn.ReLU()
    ...
    def forward(self, batch_x):   # (bs, T, Node)
        ...
        x = x.permute([0, 2, 1, 3])
        x = self.relu(self.embed_node_1(x))  # 节点升维
        x = self.embed_node_2(x)
        x = x.permute([0, 2, 1, 3])
