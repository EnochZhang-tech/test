...
class TCN_Mixhop(nn.Module):
    def __init__(self, config, adj_mx=None):
        ...
        for i in range(1):
            ...
            for j in range(1, self.layers + 1):
                ...
                #                                         self.conv_channels, dilation_factor=new_dilation))
                self.tcn.append(TCN(input_channels=self.conv_channels, kernel_size=config.ks, tcn_layers=[self.conv_channels * 2, self.conv_channels], d0=config.d0))
                ...
                #                                elementwise_affine=self.layer_norm_affline))
                self.norm.append(LayerNorm((self.residual_channels, self.num_nodes,
                                            self.input_window), elementwise_affine=self.layer_norm_affline))
    ...
    def forward(self, batch_x):   # (bs, T, Node)
        ...
        skip = self.skip0(F.dropout(inputs, self.dropout, training=self.training))   # 第一跳
